...
**Adapted role-intent step:** On a `role_routing_unavailable` surface, before each App Architect or Critic spawn, run `omx ralplan role-intent write --role <role> --parent-thread <leader-thread-id> --json`; read `spawn_task_name` from its receipt (`omx-role-intent:<correlation_token>`), then spawn the App Architect or Critic with `task_name` set to that exact App-supported value, **not** `agent_nickname`. The recorded validated intent and correlation token are the authoritative role carrier, never a prompt label.
