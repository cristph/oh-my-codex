...
async function checkPluginScopedNativeHooks(
	...
	try {
		// Plugin launchers intentionally no-op non-OMX UserPromptSubmit events. Use
		// Stop instead: its schema-safe no-op is JSON-producing without inventing
		// launch authority, while the coverage check above still verifies every
		// managed hook coordinate, including UserPromptSubmit.
	...
	try {
		...
		if (denialDetail) {
			return {
				name: "Native hook dist smoke",
				status: "fail",
				message: `installed native hook dist denied a minimal UserPromptSubmit smoke (${denialDetail}); reinstall the matching oh-my-codex version and then run "omx setup --force"`,
			};
		}
