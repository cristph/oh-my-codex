...
function shouldSkipForSpawnPermissions(error: string | undefined): boolean {
	return Boolean(error && /(?:EACCES|EPERM|operation not permitted|permission denied)/i.test(error));
}
